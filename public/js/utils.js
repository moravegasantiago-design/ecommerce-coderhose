...
export const crearTemplate = (templante)=>{   
        const contenidoHome = document.querySelector(".contenido-home");
        const content = document.querySelector(".content");
        const verificadorHome = contenidoHome.classList[contenidoHome.classList.length -1];
        const verificadorContent = content.classList[content.classList.length -1];
        if(verificadorContent === "oculto" && verificadorHome === "active"|| verificadorContent === "active" && verificadorHome === "oculto"){
        if(verificadorHome === "active"){
            contenidoHome.classList.remove("active");
            contenidoHome.classList.add("oculto");
        }
...
export const modificarFiltros = (filtrosActivos) => {
    ...
    cogerDatos().forEach(d => {
        const verificarExistencia = filtrosActivos.findIndex(f => JSON.stringify(f) === JSON.stringify(d));
        if(verificarExistencia !== -1) return;
        const clave = Object.keys(d)
        if(!Array.isArray(d[clave[0]])){
            const indice = filtrosActivos.findIndex(f => f.precioMin || f.precioMax)
...
export const clickFuera = ()=>{
    const navOverlay = document.querySelector(".nav-overlay");
        navOverlay.addEventListener("click", ()=>{
            ...
            }else if (document.querySelector(".filters-sidebar").classList.contains("active")){
                const searchContent = document.querySelector(".filters-sidebar");
                const filtersBackdrop = document.querySelector(".nav-overlay");
                searchContent.classList.remove("active")
                filtersBackdrop.classList.remove("active")
...
export const eliminarClase = ()=>{
    const inputPrice = document.querySelector(".price-range").querySelectorAll("input");
    const textError = document.querySelector(".price-error")
    const validador = inputPrice[0].classList.contains("error") || inputPrice[1].classList.contains("error");
    if(validador){
        const min = Number(inputPrice[0].value);
        const max = Number(inputPrice[1].value);
        if(inputPrice[0].value !== "" && inputPrice[1].value !== "" && min <= max){
...
export const ajustarCantidad =(eventoClick, selectorInput, selectorDiminuir, selectorAumentar) =>{
